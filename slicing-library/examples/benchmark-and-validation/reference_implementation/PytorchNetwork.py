...
def create_test_network(layer_sizes, activation="relu", device=None):
    ...
    Args:
        layer_sizes: List of layer sizes [input, hidden1, hidden2, ..., output]
                    e.g., [3, 4, 1] for 3->4->1 network
        activation: Activation function as string. Options: "relu", "sigmoid", "tanh", "linear"
        device: Device to use ("cpu", "cuda", "mps"). If None, auto-selects.
    
    Returns:
        TestNetwork instance configured to match create_mlp behavior
        
    Example:
        # Create network matching create_mlp([3, 4, 1])
        network = create_test_network([3, 4, 1], activation="relu", device="cpu")
        
        # Parameter count verification
        param_count = sum(p.numel() for p in network.parameters())
        print(f"Parameters: {param_count}")  # Should match create_mlp
    """
    # Create TestNetwork with the layer configuration
    network = TestNetwork(layersShape=layer_sizes, device=device)
    
    # Set activation function to match create_mlp behavior
    network.setActivationFunction(activation)
    
    # Loss function is already MSE by default, which matches typical usage
